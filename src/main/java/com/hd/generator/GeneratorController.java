...
public class GeneratorController {
	...
	// ===================================
	Bean bean = new Bean();
	private static String BEAN_URL = beanLoadProperties.getValue("BEAN_CONTROLLER_URL");
	...
	public void generateBean(String tableName, List<BeanProperty> beanProperties) throws Exception {
		if (beanProperties != null) {
			...
			String fileName = path + handleTableName(tableName) + "Controller.java";
			File file = new File(fileName);
			FileWriter fw = new FileWriter(file);
			
			bean.setName(handleTableName(tableName));
			log.info(handleTableName(tableName));
			bean.setBeanUrl(BEAN_URL);
			bean.setLowerName(tableName);
			fw.write(createCode(beanLoadProperties.getValue("BEAN_TEMPLATE_CONTROLLER_VM_PATH"), bean, annotation, beanProperties));
	...
	public static void main(String[] args) throws Exception{
		...
		String tableNames[]={"resource","user"};

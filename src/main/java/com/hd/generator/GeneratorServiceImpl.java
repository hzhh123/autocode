...
public class GeneratorServiceImpl {
	...
	// ===================================
	Bean bean = new Bean();
	private static String BEAN_URL = beanLoadProperties.getValue("BEAN_SERVICE_IMPL_URL");
	...
	public void generateBean(String tableName, List<BeanProperty> beanProperties) throws Exception {
		if (beanProperties != null) {
			...
			String fileName = path + handleTableName(tableName) + "ServiceImpl.java";
			...
			fw.write(createCode(beanLoadProperties.getValue("BEAN_TEMPLATE_SERVICE_IMPL_VM_PATH"), bean, annotation, beanProperties));

...
public class GeneratorWebSelect {
	...
	// ===================================
	Bean bean = new Bean();
	private static String BEAN_URL = beanLoadProperties.getValue("BEAN_WEB_URL");
	private static String BEAN_PATH = BEAN_URL;
	...
	public void generateBean(String tableName, List<BeanProperty> beanProperties) throws Exception {
		if (beanProperties != null) {
			...
			String path = System.getProperty("user.dir") + "/src/main/webapp/" + BEAN_PATH + "/"+tableName+"/";
			File filePath = new File(path);
			if (!filePath.exists()) {
				filePath.mkdirs();
				log.info("创建路径[" + path + "]成功!");
			}
			String fileName = path +tableName + "Select.jsp";
			...
			fw.write(createCode(beanLoadProperties.getValue("BEAN_TEMPLATE_WEB_SELECT_URL"), bean, annotation, beanProperties));
